      ...
      const chartLegend = chartData.map((item, i) => {
        if (`${item.name}` === "Amber") {
          ...
          <div key={i} style={{color: "#FFBF00", margin:' 0 10px'}} >
            <span style={{fontWeight:'bold',fontSize:'30px'}}>.</span>{item.name}:  ₦{(item.value).toLocaleString("en-US", {
                  maximumFractionDigits: 2,
                })}bn
          </div>
          )
        ...
        <>
         <Row>
              <div className="mt-3">
                ...
                <div style={{display:"inlineBlock"}} className='d-flex justify-content-center mr-1'>
                   { chartLegend }
