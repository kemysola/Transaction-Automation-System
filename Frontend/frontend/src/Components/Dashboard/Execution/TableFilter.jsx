...
const TableFilter = ({ props, dealFilter, staffFilter,filteredItemsDta }) => {
  ...
  useEffect(() => {
    ...
    // }
  }, [filteredItemsDta, staffFilter, filteredStore]);
...
//   };
  ...
  const getTrProps = (row, i) => {
    if (row) {
      ...
      if (`${filteredItemsDta[i].deal_category}` === "Yellow") {
        return {
          style: {
            color: "#FFBF00",
      ...
      return {
        style: {
          color: `${filteredItemsDta[i].deal_category}`,
          borderColor: "transparent",
        },
      };
    }
