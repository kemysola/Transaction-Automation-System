...
import { Container, Row, Col, ProgressBar, Card, Form, Button,Modal } from "react-bootstrap";
...
export default function Progress() {
  ...
  const filterStaffData = async (dealFilter) => {  
      const filteredData = staffData.filter(item => {return item.deal_category === dealFilter})
        setData(filteredData)
        console.log("unique staff colors", filteredData)
      return filteredData
  }
  ...
  const productChartData = [
    {
      ...
      countPercent: `${((productOption1.length/data.length) * 100).toFixed(1)}%`
    },
    {
      ...
      countPercent: `${((productOption2.length/data.length) * 100).toFixed(1)}%`
    },
    {
      ...
      countPercent: `${((productOption3.length/data.length) * 100).toFixed(1)}%`
    },
    {
      ...
      countPercent: `${((productOption4.length/data.length) * 100).toFixed(1)}%`
    },
    {
      ...
      countPercent: `${((productOption5.length/data.length) * 100).toFixed(1)}%`
    },
    {
      ...
      countPercent: `${((productOption6.length/data.length) * 100).toFixed(1)}%`
    },
  ];
    ...
    <React.Fragment>
      <Container fluid className='bg-light'>
        <Row className="d-flex justify-content-between">
          ...
          <Col sm={6} lg={4}>
            <Button variant="info" className="my-1" size="sm" onClick={handleShow}>
              Filter
            </Button>
          </Col>
        </Row>
          ...
          <Modal show={show} onHide={handleClose} backdrop={false}>
            ...
            <Modal.Body>
              <Row>
                ...
                <Col sm={4} className='mt-3 pt-2'>
                  <Form.Select size="sm" name='staff' onChange={e => setStaffFilter(e.target.value)}>
                      <option value="All">All</option>
                      {staffList.map((opt, i) => (
                        <option key={staffList[i].email} value={staffList[i].email}>{staffList[i].stafflist}</option>
                      ))}
                  </Form.Select>
                </Col>
              </Row>
              <Row>
                ...
                <Col sm={4}  className='mt-3 pt-2'>
                    ...
                    <Form.Check inline label="Value" type="radio" name='indFilter' value="Value" onClick={e => setIndFilter(e.target.value)} defaultChecked />
                    <Form.Check inline label="Count" type="radio" name='indFilter' value="Count" onClick={e => setIndFilter(e.target.value)} />
                </Col>

                  {/* Filter Product Barchart  */}
                <Col sm={4}  className='mt-3 pt-2'>
                  <Form.Label>Product by</Form.Label>
                  <Form.Check inline label="Value" type="radio" name='prdFilter' value="Value" onClick={e => setPrdFilter(e.target.value)} defaultChecked />
                  <Form.Check inline label="Count" type="radio" name='prdFilter' value="Count" onClick={e => setPrdFilter(e.target.value)} />
                  </Col>

              {/* Filter By deal Category */}
                <Col sm={4}  className='mt-3 pt-2'>
                  <Form.Label>Deal Category</Form.Label>
                  <Form.Check inline label="All" type="radio" name='dealFilter' value="All" onClick={e => setDealFilter(e.target.value)} defaultChecked />
                  <Form.Check inline label="Green" type="radio" name='dealFilter' value="Green" onClick={e => setDealFilter(e.target.value)} />
                  <Form.Check inline label="Amber" type="radio" name='dealFilter' value="Yellow" onClick={e => setDealFilter(e.target.value)} />
                  <Form.Check inline label="Red" type="radio" name='dealFilter' value="Red" onClick={e => setDealFilter(e.target.value)} />
                </Col>
              </Row>
            </Modal.Body>
      ...
      <br />
      <Table dealFilter={dealFilter} filterData={filterData} />
    </React.Fragment>
  );
}