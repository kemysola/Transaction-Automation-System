...
const funcFinancialClose = (
  ...
  if (
    ...
    return parseInt(
      mandateLetter + creditCommittee + feeLetter + originator - 100
    );
  } else {
    return parseInt(0);
  }
...
router.post("/onboard", verifyTokenAndAdmin, async (req, res) => {
  ...
  try {
    ...
    const write_to_db = `INSERT INTO TB_TRS_USERS(email, password, firstName, lastName, level, hasOriginationTarget, originationAmount, guaranteePipeline,
    ...
    const oneTimePassword = CryptoJS.AES.decrypt(
      res_.rows[0].password,
      process.env.PASSWORD_SECRET_PASSPHRASE
    ).toString(CryptoJS.enc.Utf8);
  ...
  async (req, res) => {
    ...
    try {
      ...
      const user_data = [
        ...
        user_rec.lastName,
        user_rec.level,
        user_rec.hasOriginationTarget,
        user_rec.originationAmount,
        user_rec.guaranteePipeline,
        user_rec.greenTransaction,
        user_rec.amberTransaction,
        user_rec.originator,
        user_rec.mandateLetter,
        user_rec.creditCommiteeApproval,
        user_rec.feeLetter,
        funcFinancialClose(
          user_rec.originator,
          user_rec.mandateLetter,
    ...
    if (user) {
      ...
      if (oldPassword === password) {
        ...
        res.json({
          ...
          message:
            "Invalid One-Time-Password, Please check your email and try again!",
        });
      }
    }
  } catch (e) {
...
router.post("/first_onboard", async (req, res) => {
  ...
  try {
    ...
    const user_data = [
      ...
      creditCommiteeApproval,
      feeLetter,
      funcFinancialClose(0, 0, 0, 0),
      "superadmin@trmsapp.com",
      status,
      isadmin,
    ];

    await client.query("BEGIN");
    const write_to_db = `INSERT INTO TB_TRS_USERS(
