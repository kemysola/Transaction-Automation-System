...
router.get(
  ...
  async (req, res) => {
    ...
    const reporting_params = ({ start_date, end_date, client_name } =
      req.params);
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const reportQuery = await client.query("SELECT * FROM $1", [report_name]);
      if (reportQuery) {
...
router.get("/:create_date", verifyTokenAndAuthorization, async (req, res) => {
  ...
  try {
    ...
    const returned_date = await client.query(
      "SELECT * FROM TB_INFRCR_TRANSACTION WHERE createdate::text LIKE $1",
      [create_date + "%"]
    );
    // if (staff) {
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const transaction_data = await client.query(
        "SELECT clientname, originator, transactor, transactionlegallead, industry, product, region, dealsize, coupon, tenor, moratorium, repaymentfrequency, mandateletter, creditapproval, feeletter, expectedclose, actualclose, structuringfeeamount, structuringfeeadvance, structuringfeefinal, guaranteefee, monitoringfee, reimbursible,   deal_category, closed, nbc_approval_date, nbc_submitted_date FROM TB_INFRCR_TRANSACTION"
      );
      // query staff table
      const staff_data = await client.query(
        "SELECT originationamount, guaranteepipeline, mandateletter, creditcommiteeapproval, feeletter, financialclose FROM TB_TRS_USERS"
      );
      res.status(200).send({
        status: (res.statusCode = 200),
...
router.get(
  "/ccsubmission/monthly/:start_date/:end_date",
  verifyTokenAndAuthorization,
  async (req, res) => {
    const client = await pool.connect();
    try {
    ...
    try {
      const current_fy_closed_deal = await client.query(
        ...
        with closed_by_year as(
            SELECT 		row_number() over(partition by transid order by stamp) as row_no,
            ...
            select 
                 closed_date,deal_create_date,  deal_id, clientname	
                        ,originator, transactor, transactionlegallead, industry, product, region, dealsize,coupon,tenor	,moratorium	,repaymentfrequency	
                        ,amortizationstyle	,mandateletter	,creditapproval	,feeletter	,expectedclose	,actualclose	,structuringfeeamount	
                        ,structuringfeeadvance	,structuringfeefinal	,guaranteefee	,monitoringfee	,reimbursible	,deal_category
                        ,closed,nbc_approval_date	,nbc_submitted_date	,ccsubmissiondate
            from closed_by_year
            where row_no = 1;
        `,
        [final_year_slice]
  ...
  async (req, res) => {
    ...
    try {
      const current_fy_closed_deal = await client.query(`
        with main_query as (
            SELECT 
                        row_number() over(partition by transid order by stamp) as row_no, stamp as closed_date, createdate as deal_create_date, transid	as deal_id, clientname	
                        ,originator, transactor, transactionlegallead, industry, product, region, dealsize,coupon,tenor	,moratorium	,repaymentfrequency	
                        ,amortizationstyle	,mandateletter	,creditapproval	,feeletter	,expectedclose	,actualclose	,structuringfeeamount	
                        ,structuringfeeadvance	,structuringfeefinal	,guaranteefee	,monitoringfee	,reimbursible	,deal_category
                        ,Case when closed = true then 'Yes' else 'No' End closed,nbc_approval_date	,nbc_submitted_date	,ccsubmissiondate
             ...
             select 
                     closed_date, deal_create_date, deal_id, clientname	
                        ...
                        ,structuringfeeadvance	,structuringfeefinal	,guaranteefee	,monitoringfee	,reimbursible	,deal_category
                        ,closed,nbc_approval_date	,nbc_submitted_date	,ccsubmissiondate
             from main_query where row_no = 1;
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let reportsectioncontent = JSON.parse(
        quarterly_report.rows[0].reportsectioncontent
      );
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const fin_year = req.params.fin_year;
      const report_update = [
        report_content_body.ReportSectionContent,
        quarter,
        fin_year,
      ];
